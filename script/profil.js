...
function tampilSlide(n) {
    ...
    if (n > slideLapangan.length) { slideIndex = 1 }
    if (n < 1) { slideIndex = slideLapangan.length }
...
function tampilSlideLab(n) {
    ...
    if (n > slideLab.length) { slideIndexLab = 1 }
    if (n < 1) { slideIndexLab = slideLab.length }
...
function tampilSlidePerpus(n) {
    ...
    if (n > slidePerpus.length) { slideIndexPerpus = 1 }
    if (n < 1) { slideIndexPerpus = slidePerpus.length }
    for (i = 0; i < slidePerpus.length; i++) {
        slidePerpus[i].style.display = "none";
    }
    for (i = 0; i < bijiPerpus.length; i++) {
        bijiPerpus[i].className = bijiPerpus[i].className.replace(" active-Perpus", "");
    }
